...
public class EffectSkin {
    ...
    private static final String[] textltdb = new String[]{
            Manager.Name_SV, "Idol Vip Quá!", "Idol",
        "Trùm Server Đây Rồi!"
    };
    ...
    private long lastTimeDemonFost;
...
//    }
    ...
    private void updateDemonFost() {
        try {
            if (Util.canDoWithTime(lastTimeDemonFost, 5000)) {
                List<Player> playersMap = this.player.zone.getNotBosses();
                List<Player> demonFostPlayers = new ArrayList<>();
                for (Player pl : playersMap) {
                    if (pl != null && pl.nPoint != null && !pl.isBoss && !pl.isDie()
                            && Util.getDistance(this.player, pl) <= 350
                            && pl.nPoint.isDemonFost) {
                        demonFostPlayers.add(pl);
                    }
                }
                int newBuff = 0;
                if (demonFostPlayers.size() >= 2) {
                    newBuff = Math.min(demonFostPlayers.size() * 2, 10);
                }
                for (Player pl : playersMap) {
                    if (pl != null && pl.nPoint != null && !pl.isBoss && !pl.isDie()
                            && Util.getDistance(this.player, pl) <= 350) {

                        if (pl.effectSkill != null) {
                            if (pl.nPoint.isDemonFost) {
                                EffectSkillService.gI().setDameBuff(pl, 11000, newBuff);
                            }
                            else {
                                EffectSkillService.gI().setDameBuff(pl, 11000, 0);
                            }
                            Service.gI().point(pl);
                        }
                    }
                ...
                this.lastTimeDemonFost = System.currentTimeMillis();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
