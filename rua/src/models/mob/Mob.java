...
//            }
        ...
        if (MapService.gI().isMapUpPorata(mapid)) {
            ...
            if (Util.isTrue(18, 100)) {
                ...
                ItemMap it;
                if (ItemService.gI().isPorata2(pl)) {
                    it = new ItemMap(zone, 1868, 1, x, yEnd, player.id);
                } else {
                    it = new ItemMap(zone, 933, 1, x, yEnd, player.id);
                }
        ...
        // Đồ Sao và Set Kích Hoạt
        if (!player.isPet && MapService.gI().isMapUpSKH(mapid)) {
            boolean canDrop = (player.isActive() && Util.isTrue(100, 100)) ||
                    (Manager.TEST && Util.isTrue(5, 100)) ||
                    Util.isTrue(1, 100000);

            if (canDrop) {
                // Random 50/50 giữa Đồ Sao và Set Kích Hoạt
                if (Util.isTrue(1, 2)) {
                    // Đồ Sao (50% chance)
                    ...
                    int[] opsrand = ItemService.gI().randOptionItemDoSao(player.gender);
                    int value;
                    if (Util.isTrue(10, 100)) {
                        value = 3; // 10% chance rơi số 3
                    } else {
                        value = Util.nextInt(0, 2); // 90% chance rơi số 1 hoặc 2
                    }
                    it.options.add(new Item.ItemOption(opsrand[0], value));
                    list.add(it);
                } else {
                    // Set Kích Hoạt (50% chance)
                    ...
                    // Add tối đa 4 option random (tùy theo độ dài mảng)
                    for (int i = 0; i < Math.min(opsrand.length, 4); i++) {
                        it.options.add(new Item.ItemOption(opsrand[i], 0));
                    }
        ...
        if (MapService.gI().isMapCold(mapid)) {
            ...
            if (Util.isTrue(1, 20000) || (pl.isActive() && Util.isTrue(1, 5000)) || (pl.isAdmin() && Util.isTrue(10, 100))) {
                ItemMap it = ItemService.gI().randDoTL(this.zone, 1, x, yEnd, pl.id);
                list.add(it);
                ServerNotify.gI().notify(pl.name + " vừa nhặt được " + it.itemTemplate.name + " tại " + this.zone.map.mapName + " khu " + this.zone.zoneId);
            }
            if ((Util.isTrue(1, 500) || (pl.isActive() && Util.isTrue(1, 100))) && InventoryService.gI().fullSetThan(pl)) {
                ItemMap it = new ItemMap(zone, Util.nextInt(663, 667), 1, x, yEnd, pl.id);
                it.options.add(new Item.ItemOption(30, 0));
                list.add(it);
            }
        }

        // Map Tuong Lai - Than Linh
        if (MapService.gI().isMapTuongLai(mapid) && ((Util.isTrue(1, 1000) || (player.isActive() && Util.isTrue(1, 150)))) && InventoryService.gI().fullSetThan(player)) {
        ...
        // High power player items
//        if (player.nPoint.power >= 80000000000L) {
//            if (player.zone.map.mapId == 155) {
//                list.add(new ItemMap(zone, 2055, 1, x, yEnd, player.id));
//            } else {
//                if (Util.isTrue(1, 500) || (player.isActive() && Util.isTrue(1, 100))) {
//                    list.add(new ItemMap(zone, 2051, 1, x, yEnd, player.id));
//                }
//                if (Util.isTrue(1, 1000) || (player.isActive() && Util.isTrue(1, 200))) {
//                    list.add(new ItemMap(zone, 2052, 1, x, yEnd, player.id));
//                }
//            }
//        }

        // Rare items (cuon thu thap, etc)
        if (Util.isTrue(1, 100000) || (player.isActive() && Util.isTrue(1, 10000))) {
        ...
        // Sao pha le
        if (Util.isTrue(1, 100) || (player.nPoint.isDoSPL && Util.isTrue(5, 100)) || (player.isActive() && Util.isTrue(1, 50))) {
        ...
        // Da nang cap
        if (Util.isTrue(1, 100) || (Util.isTrue(5, 100) && MapService.gI().isMapTuongLai(mapid)) || (player.isActive() && Util.isTrue(1, 50))) {
        ...
        // Vang roi (default drop - always last)
        if (Util.isTrue(1, 100) || (Manager.TEST && Util.isTrue(1, 10)) || (player.isActive() && Util.isTrue(1, 10))) {
            int vang = Util.nextInt(1000, 30000);
            if (vang < 10000) {
