...
public class NangCapBongTaiCap3 {
    ...
    public static void showInfoCombine(Player player) {
        if (player.combineNew.itemsCombine.size() == 2) {
            ...
            if (bongTai != null && manhVo != null) {
                ...
                if (InventoryService.gI().getParam(player, 31, 1868) < 9999) {
                    ...
                    npcSay += "Còn thiếu " + (9999 - InventoryService.gI().getParam(player, 31, 1868)) + " " + manhVo.template.name;
                    CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, npcSay, "Đóng");
                } else if (player.inventory.getGemAndRuby() >= player.combineNew.gemCombine && player.inventory.gold >= player.combineNew.goldCombine) {
                ...
                CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                        "Cần 1 Bông tai Porata cấp 2 và Mảnh vỡ bông tai cấp 3", "Đóng");
            }
        } else {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                    "Cần 1 Bông tai Porata cấp 2 và Mảnh vỡ bông tai cấp 3", "Đóng");
    ...
    public static void nangCapBongTai(Player player) {
        if (player.combineNew.itemsCombine.size() == 2) {
            ...
            if (bongTai != null && manhVo != null) {
                Item findItemBag = InventoryService.gI().findItemBag(player, 1884);
                if (findItemBag != null) {
                ...
                if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                    bongTai.template = ItemService.gI().getTemplate(1884);
                    bongTai.itemOptions.clear();
                    bongTai.itemOptions.add(new Item.ItemOption(72, 3));
                    CombineService.gI().sendEffectSuccessCombine(player);
                    InventoryService.gI().subParamItemsBag(player, 1868, 31, 9999);
                } else {
                    CombineService.gI().sendEffectFailCombine(player);
                    InventoryService.gI().subParamItemsBag(player, 1868, 31, 99);
                }
