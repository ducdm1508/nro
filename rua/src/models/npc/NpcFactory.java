    ...
    public static Npc createNPC(int mapId, int status, int cx, int cy, int tempId) {
        ...
        try {
            return switch (tempId) {
                ...
                case ConstNpc.Admin ->
                    new Admin(mapId, status, cx, cy, tempId, avatar);
                default ->
...
//                        }
                    ...
                    case 671 -> {
                        switch (select) {
                            case 0 -> {
                                try {
                                    // Kiểm tra lại tiềm năng trước khi bắt đầu học
                                    if (player.nPoint.tiemNang < player.LearnSkill.Potential) {
                                        ...
                                        Service.gI().sendThongBao(player,
                                                "Bạn không đủ tiềm năng để học kỹ năng. Cần thêm "
                                                        + Service.gI().formatTien(tnPotential) + " điểm nữa.");
                                        // reset trạng thái
                                    ...
                                    // Nếu đủ tiềm năng thì mới set thời gian học
                                    var bb = ItemService.gI().getTemplate(player.LearnSkill.ItemTemplateSkillId);
                                    String[] subName = bb.name.split("");
                                    byte level = Byte.parseByte(subName[subName.length - 1]);

                                    long[] time = new long[]{3000, 3000, 3000, 3000, 3000, 3000, 300000};
                                    player.LearnSkill.Time = time[level - 1] + System.currentTimeMillis();

                                    // (CHƯA trừ tiềm năng ở đây, chỉ trừ khi học xong confirmMenu)

                                    Service.gI().ClosePanel(player);
                                    NpcService.gI().createTutorial(player,
                                            NpcService.gI().getAvatar(13 + player.gender),
                                            "Con đã bắt đầu học kỹ năng, hãy chờ đủ thời gian để hoàn tất nha!");
                                } catch (Exception e) {
                                    Logger.log("Lỗi khi bắt đầu học skill: " + e.getMessage());
                                }
                            }
                            case 1 -> {
