            ...
            switch (this.mapId) {
                ...
                case 42, 43, 44, 84 -> {
                    if (player.idMark.isBaseMenu()) {
                        switch (select) {
                            ...
                            case 2 -> {
                                ...
                                if (InventoryService.gI().findItemBongTaiCap2(player)) {
                                    ...
                                    CombineService.gI().openTabCombine(player, CombineService.NANG_CAP_BONG_TAI_CAP2);
                                }
                            }
                            case 3 -> {
                            }
                            case 4 -> CombineService.gI().openTabCombine(player, CombineService.NHAP_NGOC_RONG);
                            case 5 ->{
                                //nâng cấp bông tai
                                if (InventoryService.gI().findItemBongTaiCap3(player)) {
                                    CombineService.gI().openTabCombine(player, CombineService.NANG_CHI_SO_BONG_TAI_CAP3);
                                } else {
                                    CombineService.gI().openTabCombine(player, CombineService.NANG_CAP_BONG_TAI_CAP3);
                                }
                            }
                        }
                        //làm phép nhập đá
                        ...
                        switch (player.combineNew.typeCombine) {
                            case CombineService.NANG_CAP_VAT_PHAM, CombineService.NANG_CAP_BONG_TAI_CAP2, CombineService.NANG_CHI_SO_BONG_TAI, CombineService.LAM_PHEP_NHAP_DA, CombineService.NHAP_NGOC_RONG, CombineService.NANG_CAP_BONG_TAI_CAP3, CombineService.NANG_CHI_SO_BONG_TAI_CAP3 -> {
                                if (select == 0) {
                                    CombineService.gI().startCombine(player);
                                }
                            }
