...
public class NDVSqlFetcher {
    ...
    public static Player loadById(long id) {
        ...
        try {
            ...
            // KIỂM TRA KỸ TRƯỚC KHI GỌI loadPlayer
            if (rs != null && rs.first()) {
                player = loadPlayer(rs, true);

                // KIỂM TRA player CÓ NULL KHÔNG
                if (player != null) {
                    player.isOffline = true;
                    player.idMark.setLoadedAllDataPlayer(true);
                } else {
                    System.out.println("Lỗi: loadPlayer trả về null cho player id = " + id);
                    // Có thể log chi tiết hơn hoặc throw exception
                }
            } else {
                System.out.println("Không tìm thấy player với id = " + id);
            }
            ...
            Logger.logException(NDVSqlFetcher.class, e);
            System.out.println("Lỗi khi load player id = " + id + ": " + e.getMessage());
        } finally {
            if (rs != null) {
                rs.dispose();
            }
        ...
        try {
            ...
            if (!petData.isEmpty()) {
                ...
                if (pet.typePet == 2) {
                    ...
                    // Pet khác chỉ có 4 skill, nếu có skill 5 thì xóa đi
                    if (pet.playerSkill.skills.size() > 4) {
                        // Xóa skill thứ 5 (index 4) nếu có
                        while (pet.playerSkill.skills.size() > 4) {
                            pet.playerSkill.skills.remove(4);
                        }
                    }
                }
                pet.nPoint.hp = hp;
