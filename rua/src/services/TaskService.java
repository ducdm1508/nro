...
public class TaskService {
    ...
    public boolean checkDoneTaskTalkNpc(Player player, Npc npc) {
        switch (npc.tempId) {
            ...
            case ConstNpc.LY_TIEU_NUONG -> {
                return (player.zone.map.mapId == 5 && doneTask(player, ConstTask.TASK_11_0));
            }
            ...
            case ConstNpc.BILL -> {
                return doneTask(player, ConstTask.TASK_30_7) || doneTask(player, ConstTask.TASK_31_3);
            }
            case ConstNpc.BARDOCK -> {
                return doneTask(player, ConstTask.TASK_32_2) || doneTask(player, ConstTask.TASK_32_5);
            }
        }
        return false;
    }
    ...
    public void checkDoneTaskGoToMap(Player player, Zone zoneJoin) {
        if (player.isPl()) {
            switch (zoneJoin.map.mapId) {
                ...
                case 110 -> doneTask(player, ConstTask.TASK_29_0);
                case 155 -> doneTask(player, ConstTask.TASK_30_0);
            }
        }
    }
    ...
    public void checkDoneTaskCLTX(Player player, boolean allin) {
        if (!player.isBoss && !player.isPet) {
            if (!allin) {
    ...
    public void checkDoneTaskTX(Player player) {
        if (player.isPl()) {
            doneTask(player, ConstTask.TASK_33_2);
            if (player.tayThong >= 10) {
    ...
    public void checkDoneTaskChoXuong(Player player) {
        if (player.isPl()) {
            doneTask(player, ConstTask.TASK_34_2);
        }
    }

    public void checkDoneTaskChoNuoc(Player player) {
        if (player.isPl()) {
            doneTask(player, ConstTask.TASK_34_3);
        }
    }

    public void checkDoneTaskGiangHoa(Player player) {
        if (player.isPl()) {
            doneTask(player, ConstTask.TASK_34_5);
        }
    }

    public void checkDoneTaskNRSD(Player player) {
        if (player.isPl()) {
            doneTask(player, ConstTask.TASK_31_1);
        }
    }

    public void checkDoneTaskPhaLeHoa11S(Player player) {
        if (player.isPl()) {
            doneTask(player, ConstTask.TASK_33_4);
        }
    }

    public void checkDoneTaskSuperRank(Player player) {
        if (player.isPl()) {
    ...
    public void checkDoneTaskBocHo(Player player) {
        if (player.isPl()) {
//            if (player.bocbat < 100000000000L) {
//                return;
//            }
    ...
    public void checkDoneTaskKillBoss(Player player, Boss boss) {
        if (player != null && !player.isBoss && !player.isPet) {
            ...
            switch ((int) boss.id) {
                ...
                case BossID.COOLER -> {
                    switch (boss.currentLevel) {
                        case 0 -> doneTask(player, ConstTask.TASK_29_1);
                        case 1 -> doneTask(player, ConstTask.TASK_29_2);
                    }
                }
            }
        }
    ...
    public void checkDoneTaskKillMob(Player player, Mob mob) {
        if (!player.isBoss && !player.isPet) {
            switch (mob.tempId) {
                ...
                case ConstMob.KADO -> doneTask(player, ConstTask.TASK_29_3);
                case ConstMob.TOBI, ConstMob.CABIRA -> doneTask(player, ConstTask.TASK_32_3);
            }
            //------------------
        }
    }
        ...
        if (TaskService.gI().isCurrentTask(player, idTaskCustom)) {
            ...
            switch (idTaskCustom) {
                ...
                case ConstTask.TASK_11_0:
                    npcSay(player, ConstNpc.LY_TIEU_NUONG, "Chị là nhà cái uy tín đến từ châu phi\n"
                            + "Cưng muốn về bờ cứ đến đây, chị sẽ đưa cưng về bờ!");
                    break;
                case ConstTask.TASK_11_1:
                    break;
                case ConstTask.TASK_11_2:
                    Service.gI().sendThongBao(player, "Chúc cưng sớm về bờ nhé!");
                    break;
                //--------------------------------------------------------------
                ...
                case ConstTask.TASK_12_1:
                    npcSay(player, ConstTask.NPC_NHA,
                            "Con muốn cắm sổ đỏ của ta á? Haizz, cái lão già này lại dạy hư cháu ông rồi\n"
                                    + "Con muốn thì cũng được thôi, nhưng con phải biết lựa chọn được nhà cái uy tín đấy nhé..\n"
                ...
                case ConstTask.TASK_12_2:
                    npcSay(player, ConstTask.NPC_QUY_LAO,
                            "Cuối cùng lão ấy cũng đồng ý rồi à? Tốt lắm\n"
                                    + "Ta sẽ đưa con về bờ\n"
                                    + "Cứ all-in tài cho ta kiểu gì cũng 63 ra 1");
                    break;
                //--------------------------------------------------------------
                case ConstTask.TASK_13_0:
                    npcSay(player, ConstTask.NPC_QUY_LAO,
                            "Nhặt đút vào túi, tham lam\n"
                                    + "Nhặt đưa lên của ai đây, ngu dốt\n"
                                    + "Ví dụ như tao nhá\n"
                                    + "Bây giờ mà nhặt được 20 nghìn\n"
                                    + "Ừ thì, tất nhiên tao sẽ trả lại cho người ta rơi\n"
                                    + "Nhưng mà nhặt được 20 triệu á\n"
                                    + "Còn cái nịt\n"
                                    + "Còn đúng cái nịt thôi…");
                    break;
                ...
                case ConstTask.TASK_32_2:
                    npcSay(player, ConstNpc.BARDOCK,
                            "Tốt lắm! Ta là backdoor thật ra ta đéo bị thương ta chỉ lùa các ngươi qua đây cho vui!");
                    break;
                ...
                case ConstTask.TASK_33_5:
                    int rw = 1000;
                    NpcService.gI().createTutorial(player, -1, "Bạn vừa nhận thưởng " + rw + " điểm sự kiện");
                    break;
            }
            InventoryService.gI().sendItemBags(player);
            return true;
