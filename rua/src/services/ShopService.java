...
public class ShopService {
    ...
    private static final byte Skill_SHOP = 1;
    ...
    public Shop getShop(String tagName) throws Exception {
...
//            try {
//                msg = new Message(-44);
//                msg.writer().writeByte(Skill_SHOP);
//                msg.writer().writeByte(shop.tabShops.size());
//                for (TabShop tab : shop.tabShops) {
//                    msg.writer().writeUTF(tab.name);
//                    msg.writer().writeByte(tab.itemShops.size());
...
//
        ...
        if (shop != null) {
            ...
            try {
                ...
                for (TabShop tab : shop.tabShops) {
                    ...
                    // Lọc skill chưa học
                    List<ItemShop> availableItems = new ArrayList<>();
                    for (ItemShop itemShop : tab.itemShops) {
                        String[] subName = itemShop.temp.name.split("");
                        byte level = Byte.parseByte(subName[subName.length - 1]);
                        ...
                        boolean daHoc = player.playerSkill.skills.stream()
                                .anyMatch(s -> s.template.id == skillTemplateId && s.point >= level);

                        if (!daHoc) {
                            availableItems.add(itemShop);
                        }
                    }
                    Collections.reverse(availableItems);
                    msg.writer().writeByte(availableItems.size());
                    for (int i = availableItems.size() - 1; i >= 0; i--) {
                        ItemShop itemShop = availableItems.get(i);
                        msg.writer().writeShort(itemShop.temp.id);
    ...
    private void openShopType4(Player player, String tagName, List<Item> items) {
        ...
        try {
            ...
            for (Item item : items) {
                ...
                msg.writer().writeUTF(Manager.Name_SV);
    ...
    private void learnKyNang(Player pl, ItemShop is) {
        ...
        // Lấy thông tin level từ tên item
        String[] subName = is.temp.name.split("");
        byte level = Byte.parseByte(subName[subName.length - 1]);

        // Lấy thông tin skill template và yêu cầu tiềm năng
        var skillTemplateId = SkillUtil.getTempSkillSkillByItemID(is.temp.id);
        var potentialRequired = SkillUtil.findSkillTemplate(skillTemplateId).skillss.stream()
                .filter(s -> s.point == level)
                .findFirst()
                .map(s -> (int) s.powRequire)
                .orElse(0);
        ...
        if (pl.nPoint.tiemNang < potentialRequired) {
            long tnPotential = potentialRequired - pl.nPoint.tiemNang;
            Service.gI().sendThongBao(pl, "Bạn không đủ tiềm năng để học, Cần thêm "
        ...
        long[] time = new long[]{30000, 30000, 30000, 30000, 300000, 30000, 300000};
        var timeStudy = "";
        var timeLong = time[level - 1];

        String skillName = SkillUtil.findSkillTemplate(
                SkillUtil.getTempSkillSkillByItemID(is.temp.id)
        ).name;

        timeStudy = TimeUtil.convertTime((int) (timeLong / 1000));

        String text = String.format(
                "Con có muốn học kỹ năng %s\n" +
                        " cần %s tiềm năng" +
                        " và thời gian học là %s",
                skillName,
                potentialRequired,
                timeStudy
        );
        ...
        pl.LearnSkill.Potential = potentialRequired;
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        }else if (shop.typeShop == ShopService.Skill_SHOP) {
