            ...
            if (player.newSkill.skillSelect.template.id == Skill.MA_PHONG_BA) {
                if (Util.canDoWithTime(player.newSkill.lastTimeSkillSpecial, NewSkill.TIME_GONG)) {
                    ...
                    for (Player playerMap : playersMap) {
                        ...
                        if (player.newSkill.dir == -1 && !playerMap.isDie() && Util.getDistance(player, playerMap) <= 170 && this.canAttackPlayer(player, playerMap)) {
                            player.newSkill.playersTaget.add(playerMap);

                        } else if (player.newSkill.dir == 1 && !playerMap.isDie() && Util.getDistance(player, playerMap) <= 170 && this.canAttackPlayer(player, playerMap)) {
                            player.newSkill.playersTaget.add(playerMap);
                        }
                    }
                    ...
                    if (!player.isBoss) {
                        for (Mob mobMap : player.zone.mobs) {
                            ...
                            if (player.newSkill.dir == -1 && !mobMap.isDie() && Util.getDistance(player, mobMap) <= 170) {
                                player.newSkill.mobsTaget.add(mobMap);
                                mobMap.addTemporaryEnemies(player);
                            } else if (player.newSkill.dir == 1 && !mobMap.isDie() && Util.getDistance(player, mobMap) <= 170) {
                                player.newSkill.mobsTaget.add(mobMap);
                                mobMap.addTemporaryEnemies(player);
                            }
                        }
                    ...
                    EffectSkillService.gI().startUseMafuba(player, 500);
                }
            } else {
...
//_______________________________NEW_SKILL_NOT_FOCUS_______________________________
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            case Skill.KAIOKEN: //kaioken
                ...
                if (player.nPoint.hp <= hpUse) {
                    ...
                    Service.gI().sendEffAllPlayer(player, 501, 1, 10, 10);
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        ...
        if (plInjure.gender == 1) {
            for (Item it : plAtt.inventory.itemsBody) {
                if (it != null && it.isNotNullItem() && it.isHaveOption(45)) {
                    item = it;
                    break;
                }
            }
            if (item != null) {
                int opDame = item.getParamOption(45);
                dameAttack = dameAttack + (dameAttack * opDame / 100L);
            }
        }
        if (plInjure.gender == 2) {
            for (Item it : plAtt.inventory.itemsBody) {
                if (it != null && it.isNotNullItem() && it.isHaveOption(197)) {
                    item = it;
                    break;
                }
            }
            if (item != null) {
                int opDame = item.getParamOption(197);
                dameAttack = dameAttack + (dameAttack * opDame / 100L);
            }
        }
        if (plInjure.gender == 0) {
            for (Item it : plAtt.inventory.itemsBody) {
                if (it != null && it.isNotNullItem() && it.isHaveOption(46)) {
                    item = it;
                    break;
                }
            }
            if (item != null) {
                int opDame = item.getParamOption(46);
                dameAttack = dameAttack + (dameAttack * opDame / 100L);
            }
        }

        for (Item it : plInjure.inventory.itemsBody) {
            if (it != null && it.isNotNullItem() && it.isHaveOption(198)) {
                if (plAtt.gender == 0) {
                    int reducePercent = it.getParamOption(198);
                    dameAttack -= (dameAttack * reducePercent / 100L);
                    if (dameAttack < 0) {
                        dameAttack = 0;
                    }
                }
        ...
        for (Item it : plInjure.inventory.itemsBody) {
            if (it != null && it.isNotNullItem() && it.isHaveOption(199)) {
                if (plAtt.gender == 1) {
                    int reducePercent = it.getParamOption(199);
                    dameAttack -= (dameAttack * reducePercent / 100L);
                    if (dameAttack < 0) {
                        dameAttack = 0;
                    }
        ...
        for (Item it : plInjure.inventory.itemsBody) {
            if (it != null && it.isNotNullItem() && it.isHaveOption(200)) {
                if (plAtt.gender == 2) {
                    int reducePercent = it.getParamOption(200);
                    dameAttack -= (dameAttack * reducePercent / 100L);
                    if (dameAttack < 0) {
                        dameAttack = 0;
                    }
