...
public class InventoryService {
    ...
    public void addOptionHide(Item item, int numOption) {
        int[] array = new int[]{80, 19, 3, 81, 5, 197, 45, 46 ,204 ,200 ,198 ,199, 148, 14, 101, 97, 95, 96, 106};
        ArrayList<Integer> list = new ArrayList<>();
        for (int i3 = 0; i3 < array.length; i3++) {
            list.add(array[i3]);
        }
        for (int i4 = 0; i4 < numOption; i4++) {
            int num2 = list.remove(Util.nextInt(list.size()));
            ItemOption option2 = new ItemOption(num2, Util.nextInt(1, Util.nextInt(3, 15)));
            item.itemOptions.add(option2);
        }
    ...
    public void itemBagToBody(Player player, int index) {
        ...
        if (item.isHaveOption(210)) {
            this.addOptionHide(item, item.getParamOption(210));
            item.itemOptions.remove(item.getOption(210));
            item.itemOptions.remove(item.getOption(30));
        }
    ...
    public void itemBagToPetBody(Player player, int index) {
        try {
            if (player.pet != null && player.pet.nPoint.power >= 1500000) {
                ...
                // Nếu item có option 222 thì xử lý ẩn option trước khi mặc
                if (item.isHaveOption(222)) {
                    this.addOptionHide(item, item.getParamOption(222));
                    item.itemOptions.remove(item.getOption(222));
                }
...
//        } else
    ...
    public boolean addItemList(List<Item> items, Item itemAdd) {
        ...
        if (itemAdd.template.isUpToUp) {
            for (Item it : items) {
                if (!it.isNotNullItem() || it.template.id != itemAdd.template.id || (!checkListsEqual(it.itemOptions, itemAdd.itemOptions)  && !itemAdd.isDaNangCap() && !itemAdd.isManhTS()) || it.quantity >= 100_000_000) {
                    continue;
                }

                //========================ITEM TĂNG SỐ LƯỢNG========================
                if ((itemAdd.template.id >= 1066 && itemAdd.template.id <= 1070) || itemAdd.template.id == 457
                        || itemAdd.template.id == 610 || itemAdd.template.type == 14
        ...
        for (int i = 0; i < item.itemOptions.size(); i++) {
            ...
            if (io.optionTemplate.id == 231) {
                ...
                if (randomValue <= 0.1) {
                    ...
                    // 90% như cũ
                    int[] validParams = { 21, 30, 45, 90, 180};
