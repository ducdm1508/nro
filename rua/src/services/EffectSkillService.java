...
public class EffectSkillService {
    ...
    public void setIsHalloween(Player player, int outFit, int time) {
        if (player.effectSkill != null && EventManager.HALLOWEEN) {
...
//    }
    public void finishUseMafuba(Player player) {
        if (player.effectSkill != null && player.newSkill != null && player.location != null) {
            player.effectSkill.isUseMafuba = false;
            int skillLevel = 0;
            if (player.playerSkill != null && player.playerSkill.getSkillbyId(Skill.MA_PHONG_BA) != null) {
                skillLevel = player.playerSkill.getSkillbyId(Skill.MA_PHONG_BA).point;
            }
            int baseLimit = 1;
            if (skillLevel == 2) {
                baseLimit = 2;
            ...
            int bonus = 0;
            if (player.effectSkill.typeBinh == 1) {
                bonus = 1;
            } else if (player.effectSkill.typeBinh == 2) {
                bonus = 3;
            }
            if (player.newSkill.typeItem == 1) {
                bonus = 1;
            } else if (player.newSkill.typeItem == 2) {
                bonus = 3;
            }
            int maxTargets = baseLimit + bonus;
            int baseTime = 11000 + (skillLevel * 1500);
            int timeEffect = baseTime * (player.newSkill.typeItem == 0 ? 1 : 2);
            if (timeEffect > 21000){
                timeEffect = 21000;
            }
            // Nếu có typeItem thì nhân đôi

            // ======================
            // XỬ LÝ NGƯỜI BỊ ẢNH HƯỞNG
            // ======================
            int countPlayer = 0;
            for (Player playerMap : player.newSkill.playersTaget) {
                try {
                    if (player.location != null && playerMap.location != null) {
                        ...
                        countPlayer++;
                        if (countPlayer >= maxTargets) { // ✅ giới hạn theo cấp bình
                            break;
                        }
                    }
                } catch (Exception e) {
            ...
            for (Mob mobMap : player.newSkill.mobsTaget) {
                ...
                countMob++;
                if (countMob >= maxTargets) { // ✅ áp dụng cùng giới hạn
                    break;
                }
            }
        }
    ...
    public void setDameBuff(Player player, int tiLe) {
        player.effectSkill.isDameBuff = true;
        player.effectSkill.tileDameBuff = tiLe;
    }
